---
name: code-review
description: "Review pull requests and diffs in code-guru — the Go CLI that runs AI code review across GitHub and Azure DevOps — against the rios0rios0/guide standards: Clean Architecture, the AI-backend factory and retry decorator, stub placement across the layer boundary, and chlog fragments. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `code-guru`

`code-guru` reviews pull requests with Claude Code or the Anthropic/OpenAI APIs, enforcing configurable rule files, and posts the results back to GitHub or Azure DevOps. It talks to paid model APIs and writes public review comments, so cost, prompt integrity, and comment idempotency all matter.
...
## What matters most in `code-guru`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

```
cmd/code-guru/                 entry point: DI container, Cobra wiring
cmd/azfunc/, cmd/lambda/       serverless entry points
internal/domain/
  entities/                    FileDiff, ReviewComment, ReviewResult, ReviewRequest,
                               ReviewThread, ThreadResolution, Rule, Settings, AuthToken
  repositories/                AIReviewerRepository, RulesRepository, TrivialDetector,
                               PullRequestMetadataRepository, TokenRepository, SelfUpdaterRepository
  commands/                    ReviewCommand (+ review_batches.go), ReviewAllCommand,
                               DiscoverCommand, AuthCommand, SelfUpdateCommand, VersionCommand
internal/infrastructure/repositories/
  anthropic/ claude/ openai/   AI backends behind one interface
  rules/ trivial/ prmetadata/ auth/ selfupdate/
test/domain/doubles/           stubs for domain contracts
test/infrastructure/doubles/   stubs for infrastructure-only types
```

- **Backend selection stays behind the factory.** `AIReviewerFactory` and `RulesRepositoryFactory` in the infrastructure `container.go` pick the backend from settings. A command that names `anthropic`, `claude`, or `openai` directly is a Critical finding.
- **The retry decorator is load-bearing.** `RetryingAIReviewer` re-samples on a non-JSON or transient response up to `ai.max_attempts`, reinforcing the JSON-only instruction via `ReviewRequest.Attempt`. Removing it, or letting a raw model response escape to the pull request, is a Critical finding — that text goes public.
- **Context-window overflow has a defined path**: backends raise `support.ErrContextWindowExceeded` and `review_batches.go` falls back to batching. A new backend that fails differently breaks the fallback.
...
- External `_test` packages, table-driven with `t.Run()` subtests, `t.Parallel()` at the top of the test function.
- Stubs store the last request and return canned responses — they do not assert on call order.
- A change to a prompt, a parser, or the retry policy needs a test with a representative malformed response.
...
- The three AI backend packages looking alike — that is the interface being honoured.
