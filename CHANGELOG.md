...
- added a `Debug`-level diagnostic on `HandleAzureDevOps` allowlist rejections that emits the parsed `event_type` / `pull_id` / `status` / `repo_id` / `repo_name` / `remote_url` / `project_name` plus a 4 KB head of the raw request body — useful when the ADO management notification API shows a fully populated `resource.repository` but the live HTTP body delivered to the pod parses with empty fields (live diagnosis on `Zest-Terraform/customer-clusters#12066`). The 4 KB cap keeps the log volume bounded under burst load and matches the parser's `rawResponseLogLimit`; capture surfaces only at `Debug` so production logs at `Info` level stay clean
