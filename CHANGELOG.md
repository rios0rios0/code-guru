...
- changed the `Dockerfile` runtime stage from `gcr.io/distroless/static-debian12:nonroot` to `debian:12-slim@sha256:f9c6a2fd2ddbc23e336b6257a5245e31f996953ef06cd13a59fa0a1df2d5c252` so the `claude` AI backend can exec the Claude Code CLI inside the container; the native binary is installed via `claude.ai/install.sh` against the latest stable release on every image rebuild (intentionally not version-pinned so security fixes ship without a manual bump; downloaded to a file, then executed — no `curl | bash` pipe)
