...
func (f *fakeLeaseClient) Create(_ context.Context, lease *coordinationv1.Lease, _ metav1.CreateOptions) (*coordinationv1.Lease, error) {
	...
	f.leases[lease.Name] = lease.DeepCopy()
	return lease, nil
}

func (f *fakeLeaseClient) Delete(_ context.Context, name string, _ metav1.DeleteOptions) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.deleteCalls++
	if f.deleteErr != nil {
...
func TestK8sLeaseDedup(t *testing.T) {
	...
	t.Run("should return true when Create returns AlreadyExists (another pod owns the lease)", func(t *testing.T) {
