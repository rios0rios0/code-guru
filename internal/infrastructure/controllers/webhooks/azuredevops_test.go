...
func TestHandleAzureDevOps(t *testing.T) {
	...
	t.Run("should respond 204 (No Content) when the PR is completed", func(t *testing.T) {
		...
		// below.
		d, sub := newDispatcherWithSettings(t, defaultADOSettings())
		payload := fmt.Sprintf(`{"eventType":"git.pullrequest.updated","resource":{"pullRequestId":7,"status":"completed","repository":{"id":%q,"name":"demo-repo","remoteUrl":"https://dev.azure.com/ExampleOrg/Platform/_git/demo-repo","project":{"name":"Platform"}}}}`, adoRepoUUID)
		req := httptest.NewRequest(http.MethodPost, "/webhooks/azuredevops", bytes.NewBufferString(payload))
		req.Header.Set("Authorization", adoBasicAuth(adoSecret))
		w := httptest.NewRecorder()
	...
	t.Run("should respond 202 (Accepted) when status is empty", func(t *testing.T) {
		...
		// PR is enqueued.
		d, sub := newDispatcherWithSettings(t, defaultADOSettings())
		payload := fmt.Sprintf(`{"eventType":"git.pullrequest.updated","resource":{"pullRequestId":42,"status":"","repository":{"id":%q,"name":"demo-repo","remoteUrl":"https://dev.azure.com/ExampleOrg/Platform/_git/demo-repo","project":{"name":"Platform"}}}}`, adoRepoUUID)
		req := httptest.NewRequest(http.MethodPost, "/webhooks/azuredevops", bytes.NewBufferString(payload))
		req.Header.Set("Authorization", adoBasicAuth(adoSecret))
		w := httptest.NewRecorder()
