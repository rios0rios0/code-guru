...
// same PR creation (verified live across `#12100`, `#12101`,
// `#12102` on `2026-05-01`); when both deliveries land on the same
...
func TestWebhookDedupCache_SeenRecently(t *testing.T) {
	...
	t.Run("should return true on a duplicate within the TTL window", func(t *testing.T) {
		...
		// captured in production (PR #12101).
		cache := webhooks.NewWebhookDedupCache(30 * time.Second)
		now := time.Date(2026, 5, 1, 1, 0, 0, 0, time.UTC)
		_ = cache.SeenRecently("ado:repo-id:12101", now)

		// when
		dup := cache.SeenRecently("ado:repo-id:12101", now.Add(4*time.Second))
	...
	t.Run("should refresh the timestamp and return false after the TTL has elapsed", func(t *testing.T) {
		...
		// when
		later := cache.SeenRecently("ado:repo-id:12101", now.Add(2*time.Minute))
	...
	t.Run("should treat distinct keys as independent (one PR's duplicate does not block another)", func(t *testing.T) {
		// given: PR #12101 was already enqueued
		cache := webhooks.NewWebhookDedupCache(time.Minute)
		now := time.Date(2026, 5, 1, 1, 0, 0, 0, time.UTC)
		_ = cache.SeenRecently("ado:repo-id:12101", now)

		// when: PR #12102 arrives 1 second later (different key)
		other := cache.SeenRecently("ado:repo-id:12102", now.Add(time.Second))
	...
	t.Run("should be a no-op when constructed with a zero or negative TTL (test hook)", func(t *testing.T) {
		...
		// when
		first := cache.SeenRecently("ado:repo-id:12101", now)
		second := cache.SeenRecently("ado:repo-id:12101", now.Add(time.Millisecond))
	...
	t.Run("should let a forgotten key pass through on the next call (rollback contract)", func(t *testing.T) {
		...
		// when
		cache.Forget("ado:repo-id:12101")
		retry := cache.SeenRecently("ado:repo-id:12101", now.Add(time.Second))
	...
	t.Run("should be safe under concurrent calls on the same key (only one wins)", func(t *testing.T) {
		...
		for i := 0; i < goroutines; i++ {
			go func() {
				defer wg.Done()
				if !cache.SeenRecently("ado:repo-id:12101", now) {
					mu.Lock()
					notSeenCount++
					mu.Unlock()
				}
