...
func TestIsSkinnyADOResource(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			resource: webhooks.ADOResource{
				PullRequestID: 12096,
				URL:           "https://dev.azure.com/ZestSecurity/fb485dce-2f5b-4b1b-b45a-60ff2c1295c1/_apis/git/repositories/e3555597-e951-4908-b6d9-ff28ed2ae953/pullRequests/12096",
			},
			want: true,
		},
		{
			...
			resource: webhooks.ADOResource{
				PullRequestID: 12096,
				URL:           "https://dev.azure.com/ZestSecurity/_apis/.../pullRequests/12096",
				Repository:    webhooks.ADORepository{ID: "e3555597-e951-4908-b6d9-ff28ed2ae953"},
			},
			want: false,
		},
		{
		...
		},
		{
			...
			resource: webhooks.ADOResource{
				URL: "https://dev.azure.com/ZestSecurity/_apis/.../pullRequests/0",
			},
			want: false,
		},
		{
...
func TestAppendAPIVersion(t *testing.T) {
	...
	t.Run("should append api-version on a URL without query", func(t *testing.T) {
		// given
		raw := "https://dev.azure.com/ZestSecurity/_apis/git/repositories/abc/pullRequests/1"
	...
	t.Run("should override an existing api-version query param", func(t *testing.T) {
		// given
		raw := "https://dev.azure.com/ZestSecurity/_apis/git/pullRequests/1?api-version=5.0&foo=bar"
...
func TestMergeHydratedADOResource(t *testing.T) {
	...
	t.Run("should prefer hydrated fields when both sides supply them", func(t *testing.T) {
		// given
		original := webhooks.ADOResource{PullRequestID: 12096, URL: "https://orig"}
	...
	t.Run("should fall back to original pullRequestId when hydrated body omitted it", func(t *testing.T) {
		...
		// then
		assert.Equal(t, 12096, merged.PullRequestID)
	...
	t.Run("should fetch and decode a full PR resource", func(t *testing.T) {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_, _ = w.Write([]byte(`{
				...
				"status": "active",
				"title": "smoke",
				"sourceRefName": "refs/heads/chore/code-guru-smoke-test-6",
				"targetRefName": "refs/heads/main",
				"url": "https://dev.azure.com/ZestSecurity/_apis/git/repositories/abc/pullRequests/12096",
				"repository": {
					"id": "e3555597-e951-4908-b6d9-ff28ed2ae953",
					"name": "catalog",
					"remoteUrl": "https://dev.azure.com/ZestSecurity/backend/_git/catalog",
		...
		// when
		got, err := hydrator.Hydrate(context.Background(), server.URL+"/_apis/git/pullRequests/12096", "test-pat")

		// then
		require.NoError(t, err)
		assert.Equal(t, 12096, got.PullRequestID)
		assert.Equal(t, "active", got.Status)
		assert.Equal(t, "catalog", got.Repository.Name)
		assert.Equal(t, "backend", got.Repository.Project.Name)
		assert.Equal(t, "refs/heads/chore/code-guru-smoke-test-6", got.SourceRefName)
	})
