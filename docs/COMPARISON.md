...
| `@<bot>` mention in PR comment | ✅ (`@code-guru`, the bot's own account name via `bot_identities`, or — on Azure DevOps — the bot picked from the comment box's `@`-autocomplete, matched with no configuration) | ❌ (Copilot does not respond to user replies) | ✅ (`@claude`) |
...
**Code Guru today**: Only an explicit mention (`@code-guru`, the bot's own account name when listed in `bot_identities`, or the bot picked from the Azure DevOps `@`-autocomplete) re-triggers a review, with a word-boundary check. Replies on bot threads are silently ignored unless they carry a mention.
