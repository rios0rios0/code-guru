...
go test -run TestFunctionName ./internal/support/  # Run a single test
```
...
**Review pipeline** — draft-PR skip (default; `ai.review_drafts` opts in); review-once gate (scans for the bot's completed-review marker; a mention bypasses); no-files skip; trivial-detection fast path; "reviewing" marker with RFC 3339 start timestamp; per-file patch diffs with full-unified-diff fallback (ADO); language classification (langforge) driving rule selection; closed-PR mid-flight re-check (never posts on merged/abandoned PRs); stale-comment filter (drops findings on files no longer in the latest iteration); resolved-anchor filter (re-review path); duplicate-comment dedup (file+line+body-prefix fingerprint); inline vs PR-wide comment posting; severity levels `error`/`warning`/`info`; completion annotation (verdict + inline count + summary + timestamp); batched review fallback for `ErrContextWindowExceeded` (default ON — splits the files into context-fitting batches, announces the slower run on the PR, merges the per-batch reviews); failure annotation (classified reason only — raw model output never reaches the PR — with a dedicated "PR too large for the context window" notice carrying the change's scale + split-the-PR guidance for `ErrContextWindowExceeded` **when batching is disabled or every batch failed**, a "content-safety declined" notice — cause + policy category + request-a-human/switch-model guidance — for `ErrContentSafetyRefusal`, and a "reviewer's own configuration exceeded an OS limit" notice — diff-was-never-read + narrow-`rules.categories` operator guidance — for `ErrArgumentListTooLong`; all three notices set the review-once marker); native review submission (Approve / Request Changes / Waiting-for-Author votes); three-verdict vocabulary (`approve`/`request_changes`/`comment`) plus trivial `approve`/`reject`.
...
**Re-review (mention)** — triggered by `@code-guru`, by any `bot_identities` account name (see the mention-token invariant below), OR — on Azure DevOps, with zero configuration — by picking the bot out of the comment box's `@`-autocomplete (whose `@<identity-guid>` markup is matched against the identity the bot's own PAT authenticates as, resolved from `_apis/connectionData` and cached per org); conversation walk over the bot's prior inline threads (reply chains via `InReplyToID`, depth-bounded); per-thread LLM classification `resolved`/`outstanding`/`outdated` (required before any new comments); one reply nested inside each prior thread (`ReplyToThread`); auto-close of resolved (`fixed`) / outdated (`closed`) ADO threads; layered bot self-recognition (built-in `code-guru` shapes + configured `bot_identities` + self-detection from its own annotations); self-trigger loop guard (mention handlers skip bot-authored comments).
...
**Re-review (mention) flow:** mention webhook (GitHub `issue_comment` or `pull_request_review_comment`, ADO comment event) → Worker → `ReviewCommand` with `UserMentioned=true` → fetch prior bot inline threads via `ListPullRequestComments` → assemble `ReviewThread[]` (carrying gitforge `ThreadID`) → AI prompt requires per-thread `thread_resolutions` decisions (`resolved`/`outstanding`/`outdated`) BEFORE any new `comments` → `applyThreadResolutions` posts one reply **nested inside each prior thread** via `postResolutionReply` (gitforge `ReplyToThread` when a `ThreadID` exists, fresh inline comment otherwise) + `UpdatePullRequestThreadStatus("fixed")` on resolved / `"closed"` on outdated → `dropResolvedAnchorComments` strips new comments whose anchor was already addressed, so the same finding never lands twice. First-pass reviews leave `ThreadResolutions` empty so the prompt and post-pipeline behave exactly as before.
...
**Webhook (serve) flow:** `POST /webhooks/github` (HMAC-SHA256) or `/webhooks/azuredevops` (HTTP Basic, username hardcoded `code-guru`) → source-IP CIDR allowlist → payload parsing + org/project allowlists → dedup (per-pod TTL cache, or cross-pod K8s Lease when in-cluster; skipped for PR-wide mentions, applied to GitHub inline-thread mentions under the `gh-mention:` prefix) → bounded worker pool → `Dispatcher.HandlePR` (fresh `ReviewCommand` per job). ADO org-wide subscriptions send "skinny" payloads (`{url, pullRequestId}` only) that are REST-hydrated with an SSRF guard (https + `dev.azure.com`/`*.visualstudio.com` only). GitHub App auth exchanges an RS256 JWT for cached installation tokens.
...
- `controllers/webhooks/` — `auth.go` (HMAC-SHA256 + Basic helpers, constant-time), `azuredevops_identity.go` (`ADOIdentityResolver` — `_apis/connectionData` lookup of the identity the bot's own PAT authenticates as, per-org cache, successes only; constant host + validated/escaped org slug), `github.go` / `azuredevops.go` (vendor handlers; GitHub handles `issue_comment` + `pull_request_review_comment` through the shared `submitGitHubMention` tail, ADO covers both via its single comment event; every mention path skips the bot's own comments to break infinite review loops), `dispatcher.go` (job wiring, dedup keys `gh:owner/repo:pr` / `gh-mention:owner/repo:pr` / `ado:repoID:pr`), `dedup_cache.go` (30s TTL per-pod) + `dedup_lease.go` (K8s Lease cross-pod; needs RBAC on `coordination.k8s.io/leases`; renews every 30s against a 60s lease) + `dedup_lease_client.go` (the `coordination.k8s.io/v1` REST adapter over plain `net/http` — in-cluster CA/token wiring, `MicroTime` wire format, `Status`-`reason` error classification; no `client-go`), `source_ip.go` (CIDR allowlist; header precedence CF-Connecting-IP → X-Real-IP → X-Forwarded-For → RemoteAddr), `azuredevops_hydrator.go` (skinny-payload REST hydration + SSRF host allowlist), `installation_token_exchange.go` (GitHub App JWT → installation token, `sync.Map` cache, 5m safety margin), `worker.go` (bounded pool, panic-recovering workers, graceful drain).
...
- `repositories/claude/` — shells out to `claude --print --output-format json --tools "" --system-prompt-file <tmpfile>` with the user prompt on **stdin**; captures both stdout and stderr on failure. The system prompt goes through a **file, never argv**: it embeds the whole rule corpus, and Linux caps a single argv string at `MAX_ARG_STRLEN` (128 KiB) independently of `ARG_MAX`, so a PR touching two rule-carrying languages (e.g. Go + Python, ~77 KB of language rules on top of the ~61 KB universal set) made the kernel refuse the exec with `E2BIG` before the model was reached. `writeSystemPromptFile` stages it via `os.CreateTemp` (0600, cleaned up on return); if the temp file cannot be created the backend falls back to inline `--system-prompt`, and an `E2BIG` on that path is wrapped with `support.ErrArgumentListTooLong`. The empty `--tools` is load-bearing: it removes every built-in tool from the model's scope so the review is a one-shot text completion. With tools in scope the CLI runs its agentic loop, the model spends turns on tool calls (reviewed repositories' guidelines that say "audit X with `<shell command>`" are read as instructions to execute) and exits `error_max_turns` with no review at all. `--disallowedTools` does **not** substitute — it blocks execution, not availability, so the model still emits tool calls and still burns the turns. Keep `--tools` before a following `--`-prefixed flag; it is variadic.
...
- `repositories/container.go` — `AIReviewerFactory` (settings-driven backend selection; **unknown backend silently falls back to the Claude CLI**) wrapping every backend in the `RetryingAIReviewer` decorator (`WithRetry`): re-samples on non-JSON/transient errors up to `ai.max_attempts` times, setting `ReviewRequest.Attempt` so the prompt reinforces JSON-only output on retries; the raw model output never reaches the PR. A `support.ErrContextWindowExceeded`, `support.ErrContentSafetyRefusal`, or `support.ErrArgumentListTooLong` failure is returned immediately (not retried) — all three are deterministic on identical input. Also `RulesRepositoryFactory` and the `PullRequestMetadataRepository` binding (one shared registry instance).

### Support Package (`internal/support/`)

Shared utilities: `diff_splitter.go` (parse unified diffs; `LookupChunkByPath` normalises the ADO leading `/`), `file_classifier.go` (langforge-based language → rule-category mapping), `url_parser.go` (PR URL parsing via gitforge), `prompt_builder.go` (system + user prompt assembly, including the batch "PARTIAL REVIEW" framing — see invariants below), `response_parser.go` (4-step JSON parse: strict → fenced block → string repair → `ErrUnparseableResponse`; verdict normalised to `approve`/`request_changes`/`comment`), `conversation.go` (prior bot thread walk; `IsBotAuthor` matches configured `bot_identities` + strict `code-guru` prefix shapes), `review_marker.go` (completion markers, settings-aware mention detection — `HasMention(body, identities...)` scans the built-in `MentionToken` plus the tokens `mentionTokensFor` derives from `bot_identities`; `ExtractMentionedIdentityIDs` pulls the GUIDs out of Azure DevOps' `@<identity-guid>` autocomplete markup (`IsIdentityID` gates the shape, so only canonical GUIDs ever reach a log line or an outbound lookup) and `EqualIdentityID` folds their case against the REST API's form; `DetectBotAuthors` self-detection), `verdict_mapper.go` (AI verdict → native review submission), `truncate.go` (byte-bounded truncation; `TruncateForLog` is log-injection-safe), `context_window.go` (`ErrContextWindowExceeded`, `LooksLikeContextWindowError`, `ParseContextWindowOverage` — the used/limit token figures the batch planner sizes from), `argv_limit.go` (`ErrArgumentListTooLong`, `LooksLikeArgumentListTooLong` — `errors.Is` against `syscall.E2BIG`, no string matching: the kernel raises it, no provider phrases it).
...
- **No build tags on tests.** Unit test files carry no `//go:build` constraint, so `go test ./...`, IDE runs, and the linter all see them (`make test` still passes `-tags test,unit`; untagged files compile under any tag set). A future integration suite would be the only thing to gate, with `//go:build integration`.
...
- **Tests only run under `./cmd/...`, `./pkg/...`, and `./internal/...`.** The pipelines Go test runner (`global/scripts/languages/golang/test/run.sh`) builds its package list from those three directories and only falls back to `./...` when none exist. Since this repo has `cmd/` and `internal/`, a test placed anywhere else (including the `test/` tree, which holds doubles and builders — not tests) is silently never executed by `make test` or CI.
- `cmd/code-guru/dockerfile_test.go` is a repo-level invariant test: it asserts the `FROM golang:` builder tag in `.ci/stages/40-delivery/app.Dockerfile` is never older than the `go` directive in `go.mod`. The official `golang` images set `GOTOOLCHAIN=local`, so a stale builder aborts the Docker delivery stage at `go mod download` — bump both in the same commit.
...
- **No Kubernetes client libraries.** The cross-pod webhook dedup Lease backend speaks the `coordination.k8s.io/v1` REST API directly over `net/http` (`dedup_lease_client.go`). `k8s.io/client-go` (+ `api`/`apimachinery`) was removed: it pulled ~40 modules behind it, its release train has to move in lockstep (one out-of-step `kube-openapi` bump stopped the build compiling), and `client-go` master carries a `v0.0.0-` pseudo-version that sorts below the "fixed in" version of two 2021 advisories, so `govulncheck` flagged them against current code forever. Do not reintroduce it — the four verbs the bot needs are ~250 lines of `net/http`
...
- **Magic strings** (changing any breaks behaviour across files): `**Code Guru review` (completed/failed marker → review-once gate), `**Code Guru ` (annotation marker → `DetectBotAuthors`), `@code-guru` (`MentionToken` — the always-checked FLOOR, not the whole trigger set: `bot_identities` entries derive additional mention tokens, and on ADO an `@<identity-guid>` matching the bot's own resolved identity counts too), thread status `"closed"` (`annotationThreadStatus`), Basic-auth username `code-guru`, lease prefix `code-guru-`, `.autobump.yaml`, `{project_name}`.
...
- **Lease REST adapter invariants** (`dedup_lease_client.go`): errors are classified on the API server's `Status.reason`, NOT the status code — `AlreadyExists` (contended acquire) and `Conflict` (failed delete precondition) are both HTTP 409, and conflating them would let a takeover believe it acquired. The ServiceAccount token MUST be re-read from disk (`tokenRefreshInterval`); projected tokens rotate hourly, so reading once at startup authenticates for an hour and then 401s forever. `spec` timestamps are `MicroTime` — Go's default `time.Time` marshalling emits nanoseconds, which the API server rejects for that type.
...
**Review lifecycle**
- Automatic incremental re-review on every push with auto-resolution of fixed threads (Copilot rulesets, CodeRabbit default, Claude Code Review push mode) — Code Guru's review-once gate reopens only on a mention
...
**Interaction**
- Free-form chat / Q&A on the PR and on specific lines (`@coderabbitai`, `/gemini`, `@claude`, `/ask`) — a mention only triggers a re-review (intentional non-goal per COMPARISON.md, but the market norm is drifting toward dialogue)
