...
Key config sections: `providers[]`, `ai` (backend + openai/claude/anthropic sub-configs), `rules`, `trivial`, `server` (port, webhook_secret for `serve` command), `github_app` (app_id, private_key for GitHub App auth), `bot_identities` (env `CODE_GURU_BOT_IDENTITIES`, comma-separated — account identities code-guru posts under, so re-reviews recognize prior bot threads when posting under a service account; the GitHub App `code-guru[bot]` shape and self-detected identities are always recognized).
