...
In the YAML path (`NewSettings`), env vars override YAML only for: trivial settings, `bot_identities`, `ai.max_attempts`, `ai.project_guidelines`, `ai.pr_metadata`, `ai.max_guidelines_bytes`, `ai.max_pr_description_bytes`, and `ai.anthropic.context_1m`.
